 ...
 */
module.exports = class Minifier extends Transform {
  ...
  constructor(options = {}) {
     ...
     */
    this._options = {
      binary: typeof options.binary == 'string' ? options.binary : 'php',
   ...
   */
  close() {
    if(!this._phpServer) return Promise.reject(new Error('The PHP process is not started.'));
  ...
  _transform(file, encoding, callback) {
    ...
    promise
      ...
      .then(res => {
        if(res.error) throw new Error(`HTTP error ${res.status}`);
