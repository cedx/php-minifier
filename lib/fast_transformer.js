 ...
 */
exports.FastTransformer = class FastTransformer extends Transformer {
  ...
  async listen() {
    ...
    return new Promise((resolve, reject) => {
      this._process = spawn(this._minifier.binary, ['-S', `${FastTransformer.defaultAddress}:${this._port}`, '-t', join(__dirname, '../www')]);
  ...
  async transform(script) {
    ...
    let endPoint = new URL(`http://${FastTransformer.defaultAddress}:${port}/index.php`);
