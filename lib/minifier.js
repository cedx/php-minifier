...
class Minifier extends Transform {
   ...
   */
  close() {
    if(!this._phpServer) return Promise.reject(new Error('The PHP process is already terminated.'));
  ...
  listen() {
    ...
    return new Promise((resolve, reject) => portFinder.getPort((err, port) => {
      if(err) reject(err);
        ...
        this._phpServer = {
          host,
          process: childProcess.spawn(this._options.binary, args, {shell: true})
        };

        this._phpServer.process.stdout.once('readable', resolve);
      }
    }));
  }
