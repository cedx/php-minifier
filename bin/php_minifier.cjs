  ...
  "node_modules/readdirp/index.js"(exports2, module2) {
    ...
    var readdirpPromise = (root, options = {}) => {
      return new Promise((resolve4, reject) => {
        const files = [];
        readdirp2(root, options).on("data", (entry) => files.push(entry)).on("end", () => resolve4(files)).on("error", (error) => reject(error));
      });
    };
    readdirp2.promise = readdirpPromise;
    readdirp2.ReaddirpStream = ReaddirpStream;
...
var package_default = {
  ...
  scripts: {
    prepack: "gulp",
    start: "gulp build && node bin/php_minifier.cjs --help",
...
var FastTransformer = class _FastTransformer {
  /**
  ...
  constructor(executable = "php") {
    this.#executable = (0, import_node_path.normalize)(executable);
  }
  /**
  ...
  async listen() {
    ...
    return new Promise((fulfill, reject) => {
      const root = typeof module == "undefined" ? (0, import_node_url.fileURLToPath)(new URL("../www", import_meta.url)) : (0, import_node_path.join)(__dirname, "../www");
      const args = ["-S", `${_FastTransformer.#address}:${this.#port}`, "-t", root];
      this.#process = (0, import_node_child_process.spawn)(this.#executable, args, { stdio: ["ignore", "pipe", "ignore"] });
      this.#process.on("error", reject);
      this.#process.on("spawn", () => setTimeout(() => fulfill(this.#port), 1e3));
    });
  }
  /**
  ...
  async transform(file) {
    ...
    const url = new URL(`http://${_FastTransformer.#address}:${port}/index.php`);
    url.searchParams.set("file", (0, import_node_path.resolve)(file));
...
// src/safe_transformer.ts
var import_node_child_process2 = require("node:child_process");
var import_node_path2 = require("node:path");
var SafeTransformer = class {
  /**
   * The path to the PHP executable.
   */
  #executable;
   ...
   */
  constructor(executable = "php") {
    this.#executable = (0, import_node_path2.normalize)(executable);
  }
  /**
   * Closes this transformer and releases any resources associated with it.
   * @returns Resolves when the transformer is finally disposed.
  ...
  async transform(file) {
    ...
    return new Promise((fulfill, reject) => (0, import_node_child_process2.execFile)(this.#executable, ["-w", (0, import_node_path2.resolve)(file)], { maxBuffer }, (error, stdout) => {
      if (error)
        reject(error);
      else
        fulfill(stdout);
...
var import_readdirp = __toESM(require_readdirp(), 1);
var usage = `
...
async function main() {
  const { positionals, values } = (0, import_node_util.parseArgs)({ allowPositionals: true, options: {
    ...
    mode: { short: "m", type: "string", default: "safe" /* safe */ },
    silent: { short: "s", type: "boolean", default: false },
    version: { short: "v", type: "boolean", default: false }
  } });
  if (values.help || values.version)
    return import_node_console.default.log(values.version ? package_default.version : usage.trim());
  if (!positionals.length)
    throw Error("You must provide the path to the input directory.");
  const input = (0, import_node_path3.resolve)(positionals[0]);
  try {
    await (0, import_promises.access)(input);
  } catch {
    throw Error("The input directory was not found.");
  }
  const output = positionals.length > 1 ? (0, import_node_path3.resolve)(positionals[1]) : input;
...
async function processFiles(input, output, options = {}) {
  ...
  const transformer = mode == "fast" /* fast */ ? new FastTransformer(binary) : new SafeTransformer(binary);
  for await (const file of (0, import_readdirp.default)(input, { fileFilter: `*.${extension}` })) {
    if (!silent)
      import_node_console.default.log(`Minifying: ${file.path}`);
    const script = await transformer.transform(file.fullPath);
    const path = (0, import_node_path3.join)(output, file.path);
    await (0, import_promises.mkdir)((0, import_node_path3.dirname)(path), { recursive: true });
    await (0, import_promises.writeFile)(path, script);
  }
  return transformer.close();
}
main().catch((error) => {
  import_node_console.default.error(error instanceof Error ? error.message : error);
  import_node_process.default.exitCode = 1;
});
//# sourceMappingURL=php_minifier.cjs.map
